...
const AppointmentView: FC<AppointmentViewProps> = ({ id }) => {
  const { isPending, data: appointment, isError, error } = useGetAppointmentsById(id); // Obtener la cita
  ...
  if (isError) {
    return (
      <Alert severity="error">
        Error al cargar los detalles de la cita: {error instanceof Error ? error.message : 'Error desconocido'}
      </Alert>
    );
  }
  ...
  const specialtyName = appointment.schedule?.specialty?.name || 'Especialidad no asignada';
  ...
  const scheduleDate = appointment.schedule ? new Date(appointment.schedule.scheduleDate).toLocaleString() : 'No disponible';
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 bg-white rounded-xl shadow-lg p-8 border border-gray-100">
      ...
      <TextItem text={appointment.status || 'Estado no asignado'} title="Estado de la Cita" />
